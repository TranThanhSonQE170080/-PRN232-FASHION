...
class DatabaseManager:
    ...
    async def init_db(self):
        ...
        try:
            ...
            # Phân tích URL để kiểm tra driver
            url_obj = make_url(database_url)
            driver_name = url_obj.drivername.lower()
            
            logger.info("Creating async database engine...")
            
            # Kiểm tra xem driver có hỗ trợ connection pooling không
            # SQLite với aiosqlite không hỗ trợ pooling
            supports_pooling = not any(driver in driver_name for driver in ["sqlite", "aiosqlite"])
            
            # Configure engine based on environment và driver
            engine_kwargs = {
                "echo": settings.debug,
            }
            ...
            if is_lambda or not supports_pooling:
                # Lambda hoặc driver không hỗ trợ pooling (như SQLite): Use NullPool
                engine_kwargs["poolclass"] = NullPool
                logger.info(f"Using NullPool for {'Lambda' if is_lambda else driver_name} environment")
            else:
                # Non-Lambda và driver hỗ trợ pooling (PostgreSQL/MySQL): Use QueuePool với connection pooling
